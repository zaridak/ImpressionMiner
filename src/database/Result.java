...
public class Result implements ResultDAO {
    ...
    public LinkedHashMap<String,String> getImpressionMapByID(String url){
    ...
    */
    ...
    public void printResults(ArrayList<myThread> allThreads) {
        ...
        */
        ...
        for(myThread tmpThread : this.allThreads){ //running all threads
           ...
           //     System.out.println("For gia url to kwtmpFoundTimes = "+kwtmpFoundTimes);
            ...
            if(!tmpImpression.isEmpty()){
                tmpImpression.forEach((key, value) -> { System.out.println("For keyWord ->"+key+"<- Impression is "+value); });
            }else {
        ...
        for(String keyword : keyWords){
            for(myThread thread : allThreads){
               ...
               //     System.out.println(thread.getURl()+" contains "+keyword);
                    if(kwURLNumber.containsKey(keyword)){ // an to periexei  hdh
                        kwURLNumber.replace(keyword,kwURLNumber.get(keyword)+1);
                    }
                    else{
                        kwURLNumber.put(keyword,1);
                    }
                    if(kwURLName.containsKey(keyword)){
                        kwURLName.replace(keyword,kwURLName.get(keyword)+" "+thread.getURl());
                    }else {
                        kwURLName.put(keyword,thread.getURl());
                    }

                }
            }
        }
        kwURLNumber.forEach((k,v)-> System.out.println("to keyword "+k+" uparxei se "+v+"/"+this.urlsSearched.size()+" urls"));

        kwURLName.forEach((k,v)-> System.out.println("to keyword "+k+" uparxei sta url:  "+v));
